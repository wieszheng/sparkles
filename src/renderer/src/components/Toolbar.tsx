...
export function Toolbar({ selectedDevice }: { selectedDevice: string }) {
  ...
  const handleScreenshot = async () => {
    ...
    try {
      const res = await window.api.screencap(selectedDevice, true);
      if (res) {
        toast.success("截图成功");
      } else {
        toast.error("截图失败");
  ...
  const handleClear = async () => {
    ...
    try {
      ...
      toast.success("清除操作完成");
    } catch (error) {
      toast.error("清除操作失败", error || "");
    } finally {
          ...
          >
            <div className="flex items-center space-x-4 w-full max-w-4xl">
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <motion.div
                    className="bg-card flex flex-col items-center p-4 rounded-xl shadow-lg"
                    whileHover={{
                      boxShadow:
                  ...
                  >
                    ...
                    <motion.p
                      className="text-muted-foreground text-sm text-center mb-4 max-w-xs"
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: 0.3 }}
                    >
