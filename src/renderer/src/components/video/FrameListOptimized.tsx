...
import { ChevronLeft, ChevronRight, ZoomIn, X, ChevronLeft as LeftIcon, ChevronRight as RightIcon, CheckCircle2 } from "lucide-react";
...
interface FrameListOptimizedProps {
  ...
  onNavigationClick: (e: React.MouseEvent<HTMLDivElement>, ref: React.RefObject<HTMLDivElement>, totalFrames?: number) => void;
  isLoading?: boolean;
}
...
const VirtualizedFrameList: React.FC<FrameListOptimizedProps> = ({
  ...
  const updateVisibleRange = useCallback(() => {
    ...
    const end = Math.min(frames.length, start + Math.ceil(containerWidth / itemWidth) + 4);
  ...
  const scrollToSelectedFrame = useCallback(() => {
    ...
    // 计算选中帧的位置
    const targetScrollLeft = selectedFrame * itemWidth - containerWidth / 2 + itemWidth / 2;
  ...
  const handleZoomNavigation = useCallback((direction: 'prev' | 'next') => {
    ...
    const newIndex = direction === 'prev' ? currentZoomIndex - 1 : currentZoomIndex + 1;
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 flex-1 min-h-0">
        ...
        >
          ...
          >
            {/* 左侧占位 */}
            <div style={{ width: `${visibleRange.start * 120}px`, minWidth: 0 }} />
      ...
      <div className="flex items-center gap-2 mt-2">
        ...
        <div className="flex-1">
          <div
            ...
            onClick={(e) => handleNavigationBarClick(e, containerRef, frames.length)}
          >
            {selectedFrame !== null && (
              <div
